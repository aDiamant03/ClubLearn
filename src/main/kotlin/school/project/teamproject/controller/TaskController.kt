...
class TaskController(private val taskService: TaskService) {
    ...
    fun getTasksByTeacher(@PathVariable teacherId: Long): ResponseEntity<List<Task>> {
        val tasks = taskService.getByTeacherId(teacherId)
        return ResponseEntity(tasks, HttpStatus.OK)
    }
    @GetMapping("/{id}/teacher")
    fun getTeacherByTask(@PathVariable id: Long): ResponseEntity<Any> {
        val teacher = taskService.getTeacherByTaskId(id)
